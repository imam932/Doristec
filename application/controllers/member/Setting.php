...
class Setting extends Member_Controller{
  ...
  function index()
  {
    ...
    // load page
    $data['content'] = $this->load->view('member/profile', $data, TRUE);
  ...
  public function editProfile($id)
  {
    if ($this->input->server('REQUEST_METHOD') == 'POST')
    {
      ...
      $this->form_validation->set_rules('alamat', 'Alamat', 'trim|required');
	    ...
	    {
	      $this->session->set_flashdata('error', validation_errors());
				redirect('member/setting/editProfile/' . $id, 'refresh');
	    }
      else
      {
    ...
    $data['user'] = $this->User_model->select_by_id($id);
    // load page
    $data['content']        = $this->load->view('member/profile_edit', $data, TRUE);
